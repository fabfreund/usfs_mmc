...
double minPsi = 0.0, maxPsi = 1.0, minRho = 0.0, maxRho = 5.0, minMisIdent = 0, maxMisIdent = 0.15;
int noStepsPsi = 100, noStepsRho = 100, noStepsMisIdent = 15;
...
{
	_offMLEstimate << dataSet + 1 << "\t" << myEstimate.getPsiML() << "\t" << myEstimate.getRhoML() << "\t" << myEstimate.getMisIdentML() << "\t";
...
{
	_offL2AbsEstimate << dataSet + 1 << "\t" << myEstimate.getPsil2_abs() << "\t" << myEstimate.getRhol2_abs() << "\t" << myEstimate.getMisIdentl2_abs() << "\t";
...
{
	_offL1AbsEstimate << dataSet + 1 << "\t" << myEstimate.getPsil1_abs() << "\t" << myEstimate.getRhol1_abs() << "\t" << myEstimate.getMisIdentl1_abs() << "\t";
...
void printGrid(myEstimationMethod myEstimate, int dataSet)
{
	for(int j = 0; j < (myEstimate.getPsiRange().size()); j++)
	{
		for(int i = 0; i < (myEstimate.getRhoRange().size()); i++)
		{
			...
			{
				_offGrid << dataSet + 1 << "\t" << (myEstimate.getPsiRange())[j] << "\t" << (myEstimate.getRhoRange())[i] << "\t" << (myEstimate.getMisIdentRange())[h] << "\t";
