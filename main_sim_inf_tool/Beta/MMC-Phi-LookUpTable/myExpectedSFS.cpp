...
mpreal Beta(mpreal x, mpreal y)
{
	mpreal _beta = exp( log(tgamma(x)) + log(tgamma(y)) - log(tgamma(x + y)));
	return ( _beta );
}

void computeCoalescenceRate(double alpha, int i, MatrixXmp &BinomCoeffMatrix, VectorXmp &lambda)
{
	mpreal Alpha = alpha;
	mpreal xPrime;
	mpreal one = 1.;
	...
	{
		lambda(1) = exp( log(BinomCoeffMatrix(i, 2)) + log(Beta(two - Alpha, _i - two + Alpha)) - log(Beta(two - Alpha, Alpha)));
		lambda(0) = lambda(1);
		
		for (int x = 3; x <= i; ++x)
		{
			xPrime = x;
			lambda(x-1) = exp( log(BinomCoeffMatrix(i, x)) + log(Beta(xPrime - Alpha, _i - xPrime + Alpha)) - log(Beta(two - Alpha, Alpha)));
			lambda(0) += lambda(x-1);
		}
	}
...
void buildQ(unsigned int SampleSize, double Alpha, MatrixXmp &BinomCoeffMatrix, MatrixXmp &Q, VectorXmp &lambda)
{
	for (int i = 1; i < SampleSize; ++i)
	{
		computeCoalescenceRate(Alpha, i + 1, BinomCoeffMatrix, lambda);
		Q(i, i) = -lambda(0);
		for (int j = 1; j <= i; ++j)
		{
			Q(i, i-j) = lambda(j);
...
void buildU(unsigned int SampleSize, MatrixXmp &BinomCoeffMatrix, MatrixXmp &Q, MatrixXmp &U)
{
	mpreal sum;
	mpreal one = 1.;
	mpreal entry;
...
void build_coalescentTime(unsigned int SampleSize, double Alpha, double rho, MatrixXmp &BinomCoeffMatrix, MatrixXmp &Q, MatrixXmp &U, MatrixXmp &coalD, MatrixXmp &coalV, MatrixXmp &coalL, VectorXmp &compInvA, VectorXmp &c, VectorXmp &a)
{
...
std::vector<long double> computePhi(unsigned int SampleSize, long double &totalTreeLength, double Alpha, double rho, MatrixXmp &A, MatrixXmp &BinomCoeffMatrix, MatrixXmp &Q, MatrixXmp &U, MatrixXmp &coalD, MatrixXmp &coalV, MatrixXmp &coalL, VectorXmp &compInvA, VectorXmp &c, VectorXmp &a, VectorXmp &tau, bool &success)
{
	std::vector<long double> _Phi(SampleSize-1);
	
	build_coalescentTime(SampleSize, Alpha, rho, BinomCoeffMatrix, Q, U, coalD, coalV, coalL, compInvA, c, a);
	tau = A * a;
