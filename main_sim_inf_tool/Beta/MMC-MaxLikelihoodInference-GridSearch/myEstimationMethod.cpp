...
mLLGrid(noStepsAlpha + 1, std::vector<std::vector<double>>(noStepsRho + 1, std::vector<double>(noStepsMisIdent + 1, -std::numeric_limits<double>::infinity()))),
ml1Grid_abs(noStepsAlpha + 1, std::vector<std::vector<double>>(noStepsRho + 1, std::vector<double>(noStepsMisIdent + 1, std::numeric_limits<double>::infinity()))),
ml2Grid_abs(noStepsAlpha + 1, std::vector<std::vector<double>>(noStepsRho + 1, std::vector<double>(noStepsMisIdent + 1, std::numeric_limits<double>::infinity()))),
mMaxLogLikelihood (- std::numeric_limits<double>::infinity()),
mRhoRange(noStepsRho + 1),
mAlphaRange(noStepsAlpha + 1),
...
mThetal1_abs(NAN)
{
	...
	if (noStepsAlpha == 0)
	{
		mAlphaRange[0] = MinAlpha;
	}
	else
	{
		for (int j = 0; j < noStepsAlpha + 1; ++j)
		{
			mAlphaRange[j] = MinAlpha + j*(MaxAlpha - MinAlpha)/(noStepsAlpha);
		}
	}
	...
	for (int k = 0; k < noStepsAlpha + 1; ++k)
	{
...
double computeLogLikelihood(std::vector<double> &_passPhi,
							int sampleSize,
							std::vector<double> &SFS,
							int segSites,
							double misIdent,
					   ...
					   )
{
    ...
    for (int k = 0; k < cutoff; ++k)
    {
		distance += std::pow((std::abs(observed[k] - (expected[k] * (1 - misIdent) + expected[cutoff - 1 - k] * misIdent))), scaleParameter);
    }
    return (std::pow(distance, 1.0 / scaleParameter));
}
...
std::vector<double> myEstimationMethod::getAlphaRange()
{
...
std::vector<std::vector<std::vector<double> > > myEstimationMethod::getGrid_Alpha()
{
	return (this->mGrid_Alpha);
}
...
double myEstimationMethod::getAlphaML()
{
	return (this->mAlphaML);
}
...
double myEstimationMethod::getAlphal2_abs()
{
	return (this->mAlphal2_abs);
}

double myEstimationMethod::getAlphal1_abs()
{
	return (this->mAlphal1_abs);
}
