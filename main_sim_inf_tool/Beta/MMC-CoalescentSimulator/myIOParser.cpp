...
void readcommandLineArguments(int argc, const char *argv[])
{
	...
	while (argc_i < argc)
	{
		...
		else if ( argv_i == "-a" || argv_i == "--alpha"  ){ alpha = readNextInput<double>(argc, argc_i, argv); }
...
void printHelpMessage()
{
	...
	std::cout << std::setw(20) << "-a/--alpha FLT"           << "  --  " << "Multiple merger parameter alpha defined by a single numerical constant." << std::endl;
	...
	std::cout << "MMC-CoalescentSimulator.out -s 10 -N 50000 -mu 0.0001 -r 0.2 -alpha 1.2 -SFS -seq" << std::endl;
...
void checkArgvInput()
{
	...
	if(alpha < 1 || alpha > 2)
	{
		std::cout << "Error: alpha = " << alpha << ", but needs to be [1,2]. See ReadMe file for input arguments.\n";
		printHelpMessage();
		exit(1);
	}
	...
	if(rho < 0)
	{
		std::cout << "Warning: rho = " << rho << " (negative). Coalescent might not converge.\n";
	}
	
	if(sim_moran == 1)
	{
		std::cout << "Error: Moran model simulations requested. Program does not handle this simulation type currently. See ReadMe file for input arguments.\n";
		printHelpMessage();
		exit(1);
	}
...
void readNewick()
{
	...
	while (std::getline(_infile, _line))
	{
		...
		myTree passedTree(readNewick, alpha, mu, initialPopSize, rho);
		if (print_Sequences)
		{
...
void initiateoutputSequences(int whichSim, int rep)
{
	if (print_Sequences)
	{
		...
		strcat (_Seq_file, "Alpha-");
		strcat (_Seq_file, convert_StringToChar (convert_NumberToString(alpha)));
...
void printparameters(std::ofstream * outfile, int whichSim)
{
	...
	{
		*outfile << "# Beta-Coalescent with exponential growth";
	}
	*outfile << "\n# Alpha: " << alpha;
...
char * set_filename (char outfile_Name[])
{
	...
	strcat (outfile_Name, "Alpha-");
	strcat (outfile_Name, convert_StringToChar (convert_NumberToString(alpha)));
