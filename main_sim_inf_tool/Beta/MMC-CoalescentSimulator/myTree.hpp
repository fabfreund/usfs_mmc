...
class myTree{
	...
	void mTreeWalk(int node_lable, std::vector<myNode>& AllNodes,long double rho, double theta, double alpha);
public:
	
	// BETA
	myTree(int SampleSize, double alpha, double mu, long double rho, unsigned int N);
	// MORAN
	myTree(int SampleSize, double alpha, double mu, unsigned int N, long double rho);
	// NEWICK
	myTree(std::string Newick, double alpha, double mu, unsigned int N, long double rho);
